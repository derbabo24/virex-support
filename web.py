...
<div class="page-wrap">
  <header class="site-header">""" + LOGO_HTML + """<h1>VIREX</h1><div class="sub">Secure Verification</div></header>
  <div class="card">
    <div class="status-card">
      <span class="status-icon">✅</span>
      <h2 class="success">VERIFIED</h2>
      <div class="discord-tag" style="display:inline-flex;margin-top:18px">
        <span class="discord-dot"></span>{{ username }}
      </div>
...
<div class="page-wrap">
  <header class="site-header">""" + LOGO_HTML + """<h1>VIREX</h1><div class="sub">Staff Application</div></header>
  <div class="card">
    <div class="status-card">
    ...
    <p style="text-align:center;color:var(--muted);font-size:12px;margin-top:18px">We only read your username and ID — never your password.</p>
  </div>
  <div class="footer"><a href="{{ website }}">{{ website }}</a></div>
</div>
"""
...
<div class="page-wrap">
  <header class="site-header">""" + LOGO_HTML + """<h1>VIREX</h1><div class="sub">Staff Application</div></header>
  <div class="card">
    <div class="discord-tag"><span class="discord-dot"></span>Applying as: <strong>{{ username }}</strong></div>
    <form method="POST" action="/apply/submit" id="appForm">
      ...
      <div class="form-section">
        ...
        <div class="form-row cols-2">
          <div class="form-group"><label>Age <span class="req">*</span></label><input type="number" name="age" min="13" max="99" required placeholder="e.g. 18"></div>
          <div class="form-group"><label>Timezone <span class="req">*</span></label><input type="text" name="timezone" required placeholder="e.g. CET, EST, PST"></div>
        </div>
        <div class="form-row cols-2">
          <div class="form-group"><label>Languages <span class="req">*</span></label><input type="text" name="languages" required placeholder="e.g. English, German"></div>
          <div class="form-group"><label>Weekly availability <span class="req">*</span></label>
  ...
  <div class="footer"><a href="{{ website }}">{{ website }}</a> &nbsp;•&nbsp; All applications are reviewed manually</div>
</div>
<script>
function bindCounter(id, cntId) {
  const ta = document.getElementById(id), cnt = document.getElementById(cntId);
  if (!ta || !cnt) return;
  const max = ta.getAttribute('maxlength');
  const update = () => { cnt.textContent = ta.value.length + ' / ' + max; cnt.style.color = ta.value.length > max * 0.9 ? '#ff8844' : ''; };
  ta.addEventListener('input', update); update();
}
bindCounter('prev_staff','cnt_prev'); bindCounter('why_valora','cnt_why');
bindCounter('skills','cnt_skills'); bindCounter('extra','cnt_extra');
...
<div class="page-wrap">
  ...
  <div class="card">
    <div class="status-card">
      ...
      <p style="margin-top:14px">Thank you, <strong>{{ username }}</strong>!<br>Your application has been received and will be reviewed by our team.</p>
    </div>
    <div class="highlight-box" style="margin-top:20px">📬 You'll receive a DM on Discord once a decision has been made.</div>
    <p style="text-align:center;color:var(--muted);font-size:13px;margin-top:20px">Application ID: <code style="color:var(--blue)">{{ app_id }}</code></p>
  </div>
  <div class="footer"><a href="{{ website }}">{{ website }}</a></div>
</div>
"""
...
<div class="page-wrap">
  <header class="site-header">""" + LOGO_HTML + """<h1>VIREX</h1></header>
  <div class="card">
    <div class="status-card">
      <span class="status-icon">⏳</span>
      <h2 class="warning">PENDING</h2>
      <p style="margin-top:12px">Hey <strong>{{ username }}</strong>, you already have a pending application.<br>Please wait for our team to review it before submitting a new one.</p>
    </div>
  </div>
  <div class="footer"><a href="{{ website }}">{{ website }}</a></div>
</div>
